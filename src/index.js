...
const showDogs = pupsObj => {
  pupsObj.forEach(pup => {
    ...
    // filterButtonFunctionality(pupsObj)
  });
};
...
const filterButtonFunctionality = (event) => {
  event.target.innerText == "Filter good dogs: OFF"
    ? (event.target.innerText = "Filter good dogs: ON")
    : (event.target.innerText = "Filter good dogs: OFF");

    // event.target.innerText=="Filter good dogs: ON"
    // ?(showDogs(pupsObj).filter(pup=> pup.isGoodDog==true))
    // : showDogs(pupsObj)
};

filterButton.addEventListener("click", filterButtonFunctionality);
